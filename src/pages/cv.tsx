    ...
    <motion.div variants={container} initial="hidden" animate="show" className="space-y-16">
      ...
      >
        <div className="relative z-10 space-y-6">
          ...
          >
            <ContactBadge href="mailto:eliaz@bobadilla.tech" icon={FiMail} text="eliaz@bobadilla.tech" />
      ...
      <motion.section variants={item} className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <SkillGroup
            title="Backend"
            skills={["Node.js", "Python", "Go", "NestJS", "FastAPI", "Django", "Express"]}
            color="blue"
          />
          <SkillGroup
            title="Frontend"
            skills={["React", "Next.js", "TypeScript", "TailwindCSS", "Flutter"]}
            color="purple"
          />
          <SkillGroup
            title="Cloud & DevOps"
      ...
      <motion.section variants={item} className="space-y-6">
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <ImpactCard ecosystem="JavaScript/TypeScript" downloads="500k+" projects="50+" />
        ...
        <p className="text-center text-gray-600 dark:text-gray-400">
          Contributed to: <strong>Deno, Node.js, Vercel, NumPy, Rust, MDN</strong>
        </p>
      </motion.section>
      ...
      <motion.section variants={item} className="space-y-4">
        ...
        <div className="p-6 rounded-2xl bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-gray-700">
          <h3 className="text-xl font-bold mb-2">Universidad Nacional de Ingeniería</h3>
          <p className="text-gray-600 dark:text-gray-400 mb-4">BS in Computer Science</p>
          <div className="space-y-2 text-sm">
            <div className="flex items-center gap-2">
              <FiAward className="text-yellow-500" />
              <span className="font-semibold">Top 0.2% of Python programmers in Peru (HackerRank, 2021)</span>
            </div>
...
const SkillGroup = ({ title, skills, color }: { title: string; skills: string[]; color: string }) => {
  const colorMap = {
  ...
  return (
    <div className="p-6 rounded-2xl bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700">
      <h3 className={`text-lg font-bold mb-4 bg-gradient-to-r ${colorMap[color as keyof typeof colorMap]} bg-clip-text text-transparent`}>
        {title}
      </h3>
      <div className="flex flex-wrap gap-2">
        {skills.map((skill) => (
          <span key={skill} className="px-3 py-1 text-xs font-medium bg-gray-100 dark:bg-gray-700 rounded-full">
            {skill}
          </span>
        ))}
      </div>
...
const ImpactCard = ({ ecosystem, downloads, projects }: { ecosystem: string; downloads: string; projects: string }) => (
  <div className="p-4 rounded-xl bg-gradient-to-br from-gray-50 to-white dark:from-gray-800 dark:to-gray-900 border border-gray-200 dark:border-gray-700">
    ...
    <div className="flex gap-4 text-sm">
      <div>
        <p className="text-2xl font-bold text-blue-600 dark:text-blue-400">{downloads}</p>
        <p className="text-gray-600 dark:text-gray-400">Downloads</p>
      </div>
      <div>
        <p className="text-2xl font-bold text-purple-600 dark:text-purple-400">{projects}</p>
